...
export const onGet: RequestHandler = async ({ cacheControl, cookie, env, sharedMap }) => {
  ...
  const fetch = await supabase_secret.from("accounts").select("customer_id").eq('id', user?.id);
  ...
  if (fetch.data && fetch.data[0].customer_id) {
    ...
    if (data && data[0]) {
      ...
      sharedMap.set("premium", undefined);
  } else
    sharedMap.set("premium", undefined);
...
export const handleCheckout = server$(
  async function (Yearly: boolean) {
      ...
      serverThis.env.get("PUBLIC_SUPABASE_ANON") as string,
      {
        cookies: {
          getAll() {
            ...
            // Conver Qwik Cookie Object to desire array
            for (const [name, details] of Object.entries(serverThis.cookie.getAll())) {
              cookies.push({ name, value: details.value })
            }
            return cookies
          },
    ...
    if (user) {
      ...
      const { data } = await supabase_secret.from("accounts").select("customer_id").eq('id', user.id);

      if (data && data[0].customer_id)
        customer = data[0].customer_id as string;
      else {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        line_items: [
          {
            price: serverThis.env.get(Yearly ? "YEARLY_PRICE_ID": "MONTHLY_PRICE_ID"),
            quantity: 1,
          },
        ],
        mode: 'subscription',
...
export const handleBillingPortal = server$(
  async function () {
    const serverThis = this
    // Create client on the server using cookies data
    const supabase = createServerClient(
